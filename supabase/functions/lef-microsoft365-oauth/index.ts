...
async function createBlock(args: any, userId: string) {
  confirmed(args);
  const connection = await ownConnection(userId); if (!connection || connection.status !== "active") throw new Error("Microsoft 365 calendar is not connected");
  const title = blockText(args.title, "title", 500, true)!; const purpose = blockText(args.purpose, "purpose", 2000); const key = blockText(args.idempotency_key, "idempotency_key", 200, true)!; const range = blockRange(args); const timezone = blockText(args.timezone, "timezone", 100) || "America/Sao_Paulo";
  ...
  const token = await accessToken(userId);
  const event = await graphWrite("/me/events", token, "POST", { subject: title, body: { contentType: "text", content: purpose || "Planned with LEF Assistant" }, start: { dateTime: new Date(range.start).toISOString(), timeZone: "UTC" }, end: { dateTime: new Date(range.end).toISOString(), timeZone: "UTC" }, attendees: [], showAs: "busy", sensitivity: "normal", isReminderOn: false, categories: ["LEF Assistant"], transactionId: key });
  try {
    ...
    return { block: rows[0], calendar_event_created: true };
  } catch (e) {
...
async function createRecurringBlock(args: any, userId: string) {
  ...
  const prior = await db(`assistant_calendar_blocks?owner_user_id=eq.${userId}&transaction_key=eq.${encodeURIComponent(key)}&select=*`); if (prior.length) return { block: prior[0], reused: true };
  const token = await accessToken(userId);
  const event = await graphWrite("/me/events", token, "POST", { subject: title, body: { contentType: "text", content: purpose || "Planned with LEF Assistant" }, start: { dateTime: new Date(range.start).toISOString(), timeZone: "UTC" }, end: { dateTime: new Date(range.end).toISOString(), timeZone: "UTC" }, recurrence, attendees: [], showAs: "busy", sensitivity: "normal", isReminderOn: false, categories: ["LEF Assistant"], transactionId: key });
  try {
    ...
    return { block: rows[0], calendar_series_created: true };
  } catch (error) {
    try { await graphWrite(`/me/events/${encodeURIComponent(event.id)}`, token, "DELETE"); } catch { /* best-effort compensation */ }
    throw error;
  }
...
const tools = [
  ...
  { name: "create_calendar_block", description: "Create a normal-visibility, attendee-free Outlook planning block after immediate explicit user confirmation. Use a stable idempotency key for the agreed block.", inputSchema: { type: "object", required: ["title", "starts_at", "ends_at", "idempotency_key", "confirmed"], properties: { title: { type: "string", maxLength: 500 }, purpose: { type: "string", maxLength: 2000 }, starts_at: { type: "string", format: "date-time" }, ends_at: { type: "string", format: "date-time" }, timezone: { type: "string", maxLength: 100 }, idempotency_key: { type: "string", maxLength: 200 }, check_in_id: { type: "string", format: "uuid" }, confirmed: { type: "boolean", const: true } } } },
  { name: "create_recurring_calendar_block", description: "Create one bounded, normal-visibility, attendee-free recurring Outlook series after explicit confirmation. Supports daily, weekdays, weekly, and monthly patterns under the existing calendar permission.", inputSchema: { type: "object", required: ["title", "starts_at", "ends_at", "recurrence_type", "idempotency_key", "confirmed"], oneOf: [{ required: ["end_date"] }, { required: ["number_of_occurrences"] }], properties: { title: { type: "string", maxLength: 500 }, purpose: { type: "string", maxLength: 2000 }, starts_at: { type: "string", format: "date-time" }, ends_at: { type: "string", format: "date-time" }, timezone: { type: "string", maxLength: 100 }, recurrence_type: { type: "string", enum: ["daily","weekdays","weekly","monthly"] }, interval: { type: "integer", minimum: 1, maximum: 52, default: 1 }, days_of_week: { type: "array", minItems: 1, uniqueItems: true, items: { type: "string", enum: ["monday","tuesday","wednesday","thursday","friday","saturday","sunday"] } }, day_of_month: { type: "integer", minimum: 1, maximum: 31 }, end_date: { type: "string", format: "date" }, number_of_occurrences: { type: "integer", minimum: 2, maximum: 365 }, idempotency_key: { type: "string", maxLength: 200 }, confirmed: { type: "boolean", const: true } } } },
