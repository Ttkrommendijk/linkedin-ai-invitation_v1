...
const MCP_URL = `${PROJECT_URL}/functions/v1/lef-reminders-mcp`;
...
async function createReminder(args: any, userId: string) {
  ...
  const dueOn = args?.due_on == null ? null : boundedText(args.due_on, "due_on", 10, true);
  if (dueOn && !/^\d{4}-\d{2}-\d{2}$/.test(dueOn)) throw new Error("due_on must be YYYY-MM-DD");
  if (dueAt && dueOn) throw new Error("due_at and due_on are mutually exclusive");
...
async function updateReminder(args: any, userId: string) {
  ...
  if (hasOwn(args, "due_at")) { patch.due_at = args.due_at == null ? null : timestamp(args.due_at, "due_at", true); if (patch.due_at) patch.due_on = null; }
  if (hasOwn(args, "due_on")) { patch.due_on = args.due_on == null ? null : boundedText(args.due_on, "due_on", 10, true); if (patch.due_on && !/^\d{4}-\d{2}-\d{2}$/.test(patch.due_on)) throw new Error("due_on must be YYYY-MM-DD"); if (patch.due_on) patch.due_at = null; }
...
const allTools = [
  ...
  { name: "update_reminder", description: "Update an existing reminder. Use due_on for a date-only obligation or due_at when an exact time matters.", inputSchema: { type: "object", required: ["reminder_id","confirmed"], properties: { reminder_id: { type: "string", format: "uuid" }, confirmed: { type: "boolean", const: true }, title: { type: "string", maxLength: 300 }, details: { type: ["string","null"], maxLength: 5000 }, due_at: { type: ["string","null"], format: "date-time" }, due_on: { type: ["string","null"], format: "date" }, next_review_at: { type: "string", format: "date-time" }, priority_override: { type: ["string","null"], enum: ["low","normal","high","urgent",null] }, contact_id: { type: "string", format: "uuid" }, company_id: { type: "string", format: "uuid" }, deal_id: { type: "string", format: "uuid" } } } },
  ...
  { name: "create_reminder", description: "Create or idempotently reuse a general reminder from natural-language intent. Dates and explicit priority are optional; inferred priority must include a reason when supplied.", inputSchema: { type: "object", required: ["title"], properties: {
    title: { type: "string", maxLength: 300 }, details: { type: "string", maxLength: 5000 }, due_at: { type: "string", format: "date-time" }, due_on: { type: "string", format: "date" }, next_review_at: { type: "string", format: "date-time" },
...
const allowedToolNames = new Set(["update_reminder","create_reminder","list_reminders","list_reminders_for_review","assess_reminder","record_reminder_review","snooze_reminder","complete_reminder","cancel_reminder","reopen_reminder"]);
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "GET") return json({ name: "LEF Reminders MCP", status: "authentication_required", resource: MCP_URL, authorization_servers: [AUTH_SERVER], scopes_supported: ["email"], bearer_methods_supported: ["header"], resource_metadata: `${MCP_URL}/.well-known/oauth-protected-resource` });
  ...
  if (method === "initialize") return rpcResult(id, { protocolVersion: params?.protocolVersion || "2025-06-18", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "lef-reminders", version: "1.0.0" } });
