(function initPopupCampaignController(globalObj) {
  ...
  const {
    ...
    campaignComboToggleEl,
    campaignComboPanelEl,
    campaignComboInputEl,
    campaignComboListEl,
    companyCampaignComboToggleEl,
    companyCampaignComboPanelEl,
    companyCampaignComboInputEl,
...
colorInputEl.classList.add("form-control");
    ...
    if (campaignComboToggleEl)
      linkedCampaignsListEl.appendChild(campaignComboToggleEl);
    if (campaignComboPanelEl)
      linkedCampaignsListEl.appendChild(campaignComboPanelEl);
  }
  function renderCompanyLinkedCampaignChips() {
    ...
    if (companyCampaignComboToggleEl)
      companyLinkedCampaignsListEl.appendChild(companyCampaignComboToggleEl);
    if (companyCampaignComboPanelEl)
      companyLinkedCampaignsListEl.appendChild(companyCampaignComboPanelEl);
  }
    ...
    setKnownCampaignRows(
      campaignRowsRaw
        .map((row) => ({
          campaign_id: String(row?.campaign_id || "").trim(),
          campaign_name: normalizeCampaignValue(row?.campaign_name || ""),
          color: normalizeCampaignValue(row?.color || ""),
          archived: row?.archived === true || row?.archived === true,
        }))
        .filter(
          (row) =>
            row.campaign_id && row.campaign_name && row.archived !== true,
        ),
    );
    setKnownCampaignValues(
      getKnownCampaignRows().map((row) => row.campaign_name),
    );
    if (campaignSelectEl) {
  ...
  async function renameSelectedCampaign(campaignName) {
    ...
    const result = await sendRuntimeMessage("DB_UPDATE_CAMPAIGN", {
      payload: { campaign_id: campaignId, campaign_name: nextName },
    });
    if (!result.ok) {
      throw new Error(getErrorMessage(result.error));
    }
  ...
  function getLinkedCampaignIds(scope) {
    const rows =
      scope === "company"
        ? state.companyLinkedCampaignRows
        : state.linkedPersonCampaignRows;
    return new Set(
      (rows || []).map((row) => safeTrim(row?.campaign_id)).filter(Boolean),
    );
  }
  function getCampaignComboElements(scope) {
    if (scope === "company") {
      return {
        toggleEl: companyCampaignComboToggleEl,
    ...
    return {
      toggleEl: campaignComboToggleEl,
  ...
  function closeCampaignCombo(scope) {
    const { panelEl, inputEl, listEl, selectEl } =
      getCampaignComboElements(scope);
  ...
  function getFilteredCampaignRows(scope, searchText) {
    ...
    return state.knownCampaignRows
      .filter(
        (row) => safeTrim(row?.campaign_id) && safeTrim(row?.campaign_name),
      )
      .filter((row) => !linkedIds.has(safeTrim(row?.campaign_id)))
      .filter(
        (row) =>
          !query || safeTrim(row?.campaign_name).toLowerCase().includes(query),
      )
      .slice(0, 25);
  }
  function hasExactCampaignName(searchText) {
    ...
    return state.knownCampaignRows.some(
      (row) => safeTrim(row?.campaign_name).toLowerCase() === query,
    );
  }
  ...
  async function createCampaignFromCombo(scope, campaignName) {
    ...
    try {
      ...
      if (!createResult.ok || !createResp?.campaign?.campaign_id) {
        throw new Error(
          getErrorMessage(createResult.error || createResp?.error),
        );
      }
  ...
  async function refreshCampaignUiAfterMutation(scope) {
    const refreshCompanyPeopleList = requireFn("refreshCompanyPeopleList");
    await loadCampaignOptions({ keepSelected: true });
    if (scope === "company") {
      await refreshCompanyPeopleList();
  ...
  async function updateCampaignNameFromCombo(scope, campaignId, campaignName) {
    const sendRuntimeMessage = requireFn("sendRuntimeMessage");
    const setFooterUpdatingStatus = requireFn("setFooterUpdatingStatus");
    const setFooterStatus = requireFn("setFooterStatus");
    const setFooterReady = requireFn("setFooterReady");
    ...
    const normalizedCampaignId = safeTrim(campaignId);
    const normalizedName = normalizeCampaignValue(campaignName);
    if (!normalizedCampaignId || !normalizedName) return;
    setFooterUpdatingStatus();
    try {
      const result = await sendRuntimeMessage("DB_UPDATE_CAMPAIGN", {
        payload: {
      ...
      await refreshCampaignUiAfterMutation(scope);
  ...
  async function archiveCampaignFromCombo(scope, campaignId) {
    const sendRuntimeMessage = requireFn("sendRuntimeMessage");
    const setFooterUpdatingStatus = requireFn("setFooterUpdatingStatus");
    const setFooterStatus = requireFn("setFooterStatus");
    const setFooterReady = requireFn("setFooterReady");
    ...
    try {
      const result = await sendRuntimeMessage("DB_ARCHIVE_CAMPAIGN", {
        payload: { campaign_id: normalizedCampaignId },
      });
      if (!result.ok) throw new Error(getErrorMessage(result.error));
      await refreshCampaignUiAfterMutation(scope);
  ...
  function renderCampaignEditRow(scope, row) {
    const { listEl } = getCampaignComboElements(scope);
    if (!listEl) return;
...
inputEl.classList.add("form-control");
    ...
    inputEl.setAttribute("aria-label", "Campaign name");
    const saveBtn = document.createElement("button");
    saveBtn.type = "button";
    saveBtn.className = "campaign-combo-icon-btn campaign-combo-save";
    saveBtn.textContent = "✓";
    saveBtn.title = "Save campaign name";
    const cancelBtn = document.createElement("button");
    cancelBtn.type = "button";
    cancelBtn.className = "campaign-combo-icon-btn";
    cancelBtn.textContent = "×";
    cancelBtn.title = "Cancel edit";
    const save = () =>
      updateCampaignNameFromCombo(scope, row.campaign_id, inputEl.value);
    ...
    editRowEl.appendChild(inputEl);
    editRowEl.appendChild(saveBtn);
    editRowEl.appendChild(cancelBtn);
    listEl.appendChild(editRowEl);
    setTimeout(() => {
      inputEl.focus();
      inputEl.select();
    }, 0);
  ...
  function renderCampaignComboOptions(scope) {
    ...
    for (const row of rows) {
      ...
      rowEl.className = "campaign-combo-option-row";
      const labelBtn = document.createElement("button");
      labelBtn.type = "button";
      labelBtn.className = "campaign-combo-option campaign-combo-option-label";
      labelBtn.textContent = truncateCampaignLabel(row.campaign_name, 80);
      labelBtn.title = row.campaign_name;
      labelBtn.addEventListener("click", () =>
        linkCampaignFromCombo(scope, row.campaign_id),
      );
      const editBtn = document.createElement("button");
      editBtn.type = "button";
      editBtn.className = "campaign-combo-icon-btn";
      editBtn.textContent = "✎";
      editBtn.title = "Edit campaign name";
      editBtn.setAttribute("aria-label", `Edit ${row.campaign_name}`);
      editBtn.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        listEl.innerHTML = "";
        renderCampaignEditRow(scope, row);
      });
      const archiveBtn = document.createElement("button");
      archiveBtn.type = "button";
      archiveBtn.className = "campaign-combo-icon-btn campaign-combo-archive";
      archiveBtn.textContent = "🗑";
      archiveBtn.title = "Archive campaign";
      archiveBtn.setAttribute("aria-label", `Archive ${row.campaign_name}`);
      archiveBtn.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        archiveCampaignFromCombo(scope, row.campaign_id);
      });
      rowEl.appendChild(labelBtn);
      rowEl.appendChild(editBtn);
      rowEl.appendChild(archiveBtn);
    ...
    if (normalizedSearch && !hasExactCampaignName(normalizedSearch)) {
      ...
      createEl.addEventListener("click", () =>
        createCampaignFromCombo(scope, normalizedSearch),
      );
  ...
  function bindCampaignComboEvents(scope) {
    ...
    inputEl?.addEventListener("keydown", (event) => {
      ...
      event.preventDefault();
      const firstOption = getCampaignComboElements(scope).listEl?.querySelector(
        ".campaign-combo-option",
      );
  ...
  function bindCampaignEvents() {
    ...
    renameCampaignBtnEl?.addEventListener("click", () => {
      ...
      if (renameCampaignNameEl) {
        renameCampaignNameEl.value = normalizeCampaignValue(
          row?.campaign_name || "",
        );
