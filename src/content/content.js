(function initContentScript(globalObj) {
  if (globalObj.__LEF_CONTENT_SCRIPT_LOADED__) {
    return;
  }

  globalObj.__LEF_CONTENT_SCRIPT_LOADED__ = true;
  ...
  function buildCompanyPageExcerpt(companyName, rawMainText, rawBodyText = "") {
    ...
    if (filtered.length) {
      ...
      const sourceLines =
        companyIndex >= 0
          ? lines.slice(companyIndex, companyIndex + 36)
          : lines;
  ...
  function extractProfile() {
    ...
    let headline =
      ...
      cleanText(
        document.querySelector('[data-anonymize="headline"]')?.innerText,
      );
  ...
  function extractChatHistoryFromInteropShadow() {
    ...
    items.forEach((li, liIndex) => {
      const headingEl = li.querySelector(
        "time.msg-s-message-list__time-heading",
      );
      if (headingEl) {
  ...
  document.addEventListener("readystatechange", () => {
    timingLog("document_readystatechange", {
      ready_state: document.readyState,
    });
    if (
      ...
      timingLog("main_container_available", {
        ready_state: document.readyState,
      });
    }
  });
