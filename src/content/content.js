...
(function initLefWhatsappContentBridge() {
  const LEF_WA_OUTBOUND_TYPE = "LEF_WA_ACTIVE_CHAT_PHONE";
  const LEF_WA_REQUEST_TYPE = "LEF_WA_REQUEST_ACTIVE_CHAT";

  function isWhatsappWebPage() {
  ...
  function getState() {
    const state = window.__LEF_WA_CONTENT_STATE__ || {
    ...
    window.__LEF_WA_CONTENT_STATE__ = state;
    return state;
  }

  function requestWhatsappActiveChat(reason = "request") {
    if (!isWhatsappWebPage()) return 0;
    const state = getState();
    const requestId = Date.now();
    state.pendingRequestId = requestId;
    window.postMessage(
      {
        type: LEF_WA_REQUEST_TYPE,
        source: "LEF_EXTENSION_CONTENT",
        reason,
        requestId,
      },
      "*",
  ...
  function requestWhatsappBridgeInjection() {
    const state = getState();
  ...
  function handleWhatsappBridgeMessage(event) {
    ...
    if (data.type !== LEF_WA_OUTBOUND_TYPE) return;

    const state = getState();
    const phone = normalizeWhatsappPhone(data.phone);
    const extractReason = data.extractReason || data.extract_reason || "";
    const requestId = Number(data.requestId || state.pendingRequestId || 0);
    state.lastPayload = {
      phone,
      url: data.url || location.href,
      reason: data.reason || "bridge",
      extract_reason: extractReason,
      extractReason,
      titleText: cleanText(data.titleText || ""),
      historyChatId: cleanText(data.historyChatId || ""),
      extractPath: cleanText(data.extractPath || ""),
      requestId,
      receivedAt: Date.now(),
    };
    state.lastRequestId = requestId;

    if (!phone || phone === state.lastPhone) return;
  ...
  window.__LEF_WA_CONTENT_API__ = {
    requestActiveChat(reason = "api_request") {
      requestWhatsappBridgeInjection();
      return requestWhatsappActiveChat(reason);
    },
    getLastPayload() {
      return getState().lastPayload;
    },
  };
  ...
  if (!state.listenersInstalled) {
    ...
    window.addEventListener("message", handleWhatsappBridgeMessage);
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg?.type === "EXTRACT_WHATSAPP_ACTIVE_CHAT") {
    const requestId =
      window.__LEF_WA_CONTENT_API__?.requestActiveChat("runtime_request") || 0;
    setTimeout(() => {
      const payload = window.__LEF_WA_CONTENT_API__?.getLastPayload?.() || null;
      const isFresh = Boolean(
        payload?.phone && requestId && payload.requestId === requestId,
      );
      sendResponse({
        ok: isFresh,
        phone: isFresh ? payload.phone : "",
        url: isFresh ? payload.url || location.href : location.href,
